...
test <- ds %>% group_by(decade, sex) %>% summarise(avgAgeDecade = mean(average_life_expectancy, na.rm = TRUE))

g <- ggplot(test, aes(x = decade, fill = sex, weight=avgAgeDecade))
g + geom_bar(position = "dodge") + xlab("Decade")  +scale_fill_discrete(name = "Gender")
